<template>
  <div class="content pad10">
    ...
    >
      ...
      <el-table-column prop="value" header-align="center" align="center" label="值"></el-table-column>
...
export default {
  ...
  methods: {
    // 获取数据列表
    getDataList() {
      this.paramsCode = localStorage.getItem("dicitemCode");

      this.dataListLoading = true;
      this.isLoading = true;
      this.$http({
        url: this.$http.adornUrl("/sys/dicitem/list"),
        method: "get",
        params: this.$http.adornParams({
          page: this.pageIndex,
          limit: this.pageSize,
          name_like: this.dataForm.name,
          code_eq: this.paramsCode
        })
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    addOrUpdateHandle(id) {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdate.init(id, this.paramsCode);
      });
    },
    // 删除
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl("/sys/dicitem/delete"),
            method: "post",
