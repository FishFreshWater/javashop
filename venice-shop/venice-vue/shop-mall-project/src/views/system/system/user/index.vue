  ...
  <div class="content pad10">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" v-if="isAuth('sys:user:update')" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          <el-button type="text" v-if="isAuth('sys:user:delete')" size="small" @click="deleteHandle(scope.row.id, scope.row.username)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      deleteHandle (id, username) {
        ...
        this.$confirm(`确定对[用户名=${userNames.join(',')}]进行[${userNames ? '删除' : '批量删除'}]操作?`, '提示', {
