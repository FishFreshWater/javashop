  ...
  <div>
    <div style="margin-bottom: 10px;padding-left: 20px;">
        <span v-if="!dataForm.id">
          <i class="el-icon-plus">新增首页Banner</i>
        </span>
        <span v-else>
          <i class="el-icon-edit">修改首页Banner</i>
        </span>
      </div>
    <el-form size="small" :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="链接" prop="link">
        <el-input v-model="dataForm.link" disabled="disabled" placeholder="无"></el-input>
        <el-button style="margin-top: 10px;" type="primary" size="mini" @click="getLink">选择链接</el-button>
      </el-form-item>
    ...
    <get-link v-if="getLinkVisible" ref="getLinkDia" @slclink="slcedLink"/>
  </div>
</template>

<script>
import API from "@/api";
import GetLink from "../sh-tools/getlink";
export default {
  props: ["id"],
  mounted() {
    this.getData();
  ...
  methods: {
      ...
      if (this.id != -1) {
        this.$http({
          ...
          if (data && data.code === 0) {
            ...
            this.dataForm.name = data.shAdvert.id;            
    ...
    getLink() {
      this.getLinkVisible = true;
      this.$nextTick(() => {
        this.$refs.getLinkDia.init();
      });
    },
    slcedLink(path) {
      this.dataForm.link = path;
    },
    // 表单提交
    dataFormSubmit() {
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              this.$message({
                ...
                duration: 1500,
                onClose: () => {
                  this.$router.push({ path: `/shop-mall/sh-advert` });
                }
              });
            } else {
